...
Supports arbitrary find/replace including support for regular expressions, and
options for sequential numbering with start, step and padding options.
"""
...
class RenameTools():
	...
	def __init__(self):
		self.winTitle = "Rename Tools"
		self.winName = "mjbRenameToolsWindow"
		self.gMainProgressBar = mel.eval('$tmp = $gMainProgressBar')

		# Presets for renaming - will add more as and when we think of them
		self.presetItemList = [
			"None", 
			"Clean up mangled FBX node names", 
			"Clean up copy & pasted nodes", 
	...
	def UI(self):
		...
		self.advOptPanelUI("advOptPanel", "windowRoot", collapse=False)
	...
	def scopePanelUI(self, name, parent, collapse=False):
		...
		"""
		mc.frameLayout(width=400, collapsable=True, cl=collapse, label="Scope")
	...
	def renumberPanelUI(self, name, parent, collapse=False):
		...
		"""
		mc.frameLayout(width=400, collapsable=True, cl=collapse, label="Renumber")
		mc.columnLayout(name)
		mc.separator(height=4, style="none")
		mc.rowLayout(numberOfColumns=1, columnAttach1="left", columnAlign1="both", columnOffset1=142)
	...
	def advOptPanelUI(self, name, parent, collapse=False):
		...
		"""
		mc.frameLayout(width=400, collapsable=True, cl=collapse, label="Advanced Options")
		mc.columnLayout(name)
		mc.separator(height=4, style="none")
...
shelfCommandMEL = '''
    shelfButton
        ...
        -image "icon-rename.png" 
        -image1 "icon-rename.png" 
        -style "iconOnly" 
        -marginWidth 1
        -marginHeight 1
        -command "import mjbRenameTools\nmjbRenameTools.RenameTools().UI()" 
        -sourceType "python" 
        -doubleClickCommand "import mjbRenameTools\nreload(mjbRenameTools)" 
        -commandRepeatable 1
        -flat 1
    ;
'''